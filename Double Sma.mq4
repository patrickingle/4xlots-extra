...
// See more at: https://code.tutsplus.com/tutorials/create-a-algorithm-trading-robot-the-basics-of-writing-a-expert-advisor-in-mql4--cms-27984
...
double LotsOptimized()
  {
  /**
   * COMMENTING FOR 4XLOTS IMPLEMENTATION
   *
   double lot = Lots;
   ...
   return(lot);
   **/
   double lot = LotsOptimize(2000, false);
   return (lot);
  }
...
int start()
{
  int cnt=0, ticket=0, total=0;
  double shortSma=0.0, longSma=0.0, ShortSL=0.0, ShortTP=0.0, LongSL=0.0, LongTP=0.0;
    ...
    if (OrderSelect(cnt, SELECT_BY_POS, MODE_TRADES) == true) {
       if(OrderType() <= OP_SELL && OrderSymbol() == Symbol()){
         ...
         if(OrderType() == OP_BUY){
           ...
           if(isCrossed == 2){
             // Close the position
             if (OrderClose(OrderTicket(), OrderLots(), Bid, 3, Violet) == true){
               return(0);
             } 
           }
         }
         else //Look for short positions - inverse of prior conditions
         {
           ...
           if(isCrossed == 1){
             // Close the position
             if (OrderClose(OrderTicket(), OrderLots(), Ask, 3, Violet) == true) { 
               return(0);
             }
           }
         }
